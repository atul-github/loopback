...
module.exports = function(registry) {
  ...
  PersistedModel.diff = function (since, remoteChanges, options, callback) {
    var Change = this.getChangeModel();
    Change.diff(this.modelName, since, remoteChanges, options, callback);
  };
  ...
  PersistedModel.changes = function(since, filter, options, callback) {
    ...
    Change.find(changeFilter, options, function(err, changes) {
      ...
      model.find(filter, options, function(err, models) {
        if (err) return callback(err);
  ...
  PersistedModel.checkpoint = function(options, cb) {
    var Checkpoint = this.getChangeModel().getCheckpointModel();
    Checkpoint.bumpLastSeq(options, cb);
  };
  ...
  PersistedModel.currentCheckpoint = function(options, cb) {
    var Checkpoint = this.getChangeModel().getCheckpointModel();
  ...
  PersistedModel.createUpdates = function(deltas, options, cb) {
    var Change = this.getChangeModel();
    var updates = [];
    var Model = this;
    var tasks = [];
      ...
      switch (type) {
        ...
        case Change.UPDATE:
          tasks.push(function(cb) {
            Model.findById(change.modelId, options, function(err, inst) {
              if (err) return cb(err);
              if (!inst) {
                return cb &&
  ...
  PersistedModel.bulkUpdate = function(updates, options, callback) {
    ...
    buildLookupOfAffectedModelData(Model, updates, options, function(err, currentMap) {
      if (err) return callback(err);
  ...
  function buildLookupOfAffectedModelData(Model, updates, options, callback) {
    if (!cb && typeof options === 'function') {
      cb = options;
      options = {};
    }
    ...
    Model.find({where: whereAffected}, options, function(err, affectedList) {
      if (err) return callback(err);
      var dataLookup = {};
  ...
  function applyUpdate(Model, id, current, data, change, conflicts, options, cb) {
    ...
    if (rev !== change.prev) {
      ...
      return Change.rectifyModelChanges(Model.modelName, [id], options, cb);
    }
  ...
  function applyCreate(Model, id, current, data, change, conflicts, options, cb) {
    ...
    Model.create(data, options, function(createErr) {
      ...
      Model.findById(id, options, function(findErr, inst) {
        if (findErr || !inst) {
  ...
  function rectifyOnSave(ctx, next) {
    ...
    if (id != null) {
      ctx.Model.rectifyChange(id, options, reportErrorAndNext);
    } else {
      ctx.Model.rectifyAllChanges(options, reportErrorAndNext);
    }
  ...
  PersistedModel.rectifyAllChanges = function (options, callback) {
    if (!callback && typeof options === 'function') {
      callback = options;
      options = {};
    }
    options = options || {};
    this.getChangeModel().rectifyAll(options, callback);
  };
  ...
  PersistedModel.findLastChange = function(id, options, cb) {
    var Change = this.getChangeModel();
    Change.findOne({where: {modelId: id}}, options, cb);
  };

  PersistedModel.updateLastChange = function(id, data, options, cb) {
    var self = this;
    if (!cb && typeof options === 'function') {
      cb = options;
      options = {};
    ...
    options = options || {};
    this.findLastChange(id, options, function(err, inst) {
      if (err) return cb(err);
      if (!inst) {
   ...
   */
  PersistedModel.prototype.fillCustomChangeProperties = function(change, options, cb) {
    // no-op by default
    cb();
  };
